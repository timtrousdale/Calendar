...
angular.module('CalendarApp').controller('calendarController', function(){
    ...
    var startDay = vm.today.getDay() + 1;
    vm.monthList = [];
    ...
    vm.yearMaker = function(input){
        for (p = 0; p < input.length; p++){
            var monthDays = vm.monthMaker(startDay, input[p].daysInMonth);
    ...
    vm.newYear = function(){
        ...
        if (vm.months[0] == null) {
            vm.errorMessage = {
                message: 'Please enter a Month and Year into the drop down boxes.',
                cssClass: 'error'
            };
        }
    }
